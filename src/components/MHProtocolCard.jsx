...
import { mhHotline, mhSigns, mhSteps, mhPostAcute } from '../data/mh_protocol';
        ...
        <div className="space-y-4">
            <div className="bg-rose-600 text-white p-4 rounded-lg shadow-lg flex items-center gap-3">
                ...
                <div className="flex-1">
                    <h2 className="text-2xl font-black uppercase tracking-wider">Malignant Hyperthermia</h2>
                    <p className="text-rose-100 text-xs font-medium">Pediatric Anesthesia Pearls — full protocol</p>
                </div>
                <a
                    href={mhHotline.url}
                    target="_blank"
                ...
                >
                    <span className="text-[10px] uppercase tracking-wide">{mhHotline.label}</span>
            ...
            <div className="bg-white border border-rose-100 rounded-lg p-4">
                <h3 className="font-bold text-rose-800 border-b border-rose-100 pb-2 mb-2">Early Signs</h3>
                <ul className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-1 text-sm text-slate-700 list-disc pl-5">
                    {mhSigns.map(s => <li key={s}>{s}</li>)}
                </ul>
            </div>
            ...
            <div className="bg-rose-50 border border-rose-200 rounded-lg p-4">
                <h3 className="font-bold text-rose-800 border-b border-rose-200 pb-2 mb-2">
                    Quick doses for this patient ({fmt(w)} kg)
                </h3>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-sm">
                    <div className="bg-white p-2 rounded border border-rose-200">
                        <div className="text-[10px] font-bold text-rose-600 uppercase">Dantrolene first dose</div>
                    ...
                    <div className="bg-white p-2 rounded border border-rose-200">
                        <div className="text-[10px] font-bold text-rose-600 uppercase">Dantrolene maintenance</div>
                        <div className="text-lg font-bold text-rose-800">{fmt(dantroleneMaint)} mg q6h</div>
                        <div className="text-[10px] text-slate-500 font-mono">1 mg/kg once stable</div>
                    </div>
                    <div className="bg-white p-2 rounded border border-rose-200">
                    ...
                    <div className="bg-white p-2 rounded border border-rose-200">
                        <div className="text-[10px] font-bold text-rose-600 uppercase">Insulin</div>
            ...
            <div className="bg-white border border-slate-200 rounded-lg p-4">
                <h3 className="font-bold text-slate-700 border-b border-slate-200 pb-2 mb-2">After the acute phase</h3>
                <ul className="text-xs text-slate-600 list-disc pl-5 space-y-1">
                    {mhPostAcute.map((t, i) => <li key={i}>{t}</li>)}
                </ul>
            </div>
        </div>
    );
