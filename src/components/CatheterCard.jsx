...
import { arterialCatheters, centralVenousCatheters, calcCvlDepth, catheterPearls, catheterPearlsJa } from '../data/catheters';
...
const CatheterCard = () => {
    ...
    const pearls = lang === 'ja' ? catheterPearlsJa : catheterPearls;
    ...
    return (
        <div className="bg-white border border-slate-200 rounded-lg shadow-sm p-4 space-y-4">
            <h3 className="font-bold text-slate-700 flex items-center gap-2 border-b pb-2">
                <Cable size={18} /> {t('Intravascular Catheter Sizes', '血管内カテーテルサイズ')}
            </h3>

            {/* Arterial */}
            <div>
                <h4 className="text-sm font-bold text-slate-700 flex items-center gap-2 mb-1">
                    <Activity size={16} className="text-rose-500" /> {t('Arterial line', '動脈ライン')}
                </h4>
                <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                        <thead className="bg-slate-50 text-[10px] uppercase text-slate-500 font-bold">
                            <tr>
                                <th className="text-left py-1 pl-3">{t('Patient', '患者')}</th>
                                <th className="text-left py-1">{t('Catheter', 'カテーテル')}</th>
                                <th className="text-left py-1 pr-3">{t('Comment', 'コメント')}</th>
                            </tr>
                        </thead>
                        <tbody>
                            {arterialCatheters.map((row, i) => (
                                <tr key={i} className={`border-t border-slate-100 ${i === aRow ? 'bg-rose-50' : ''}`}>
                                    <td className="py-1.5 pl-3 font-bold text-slate-800">{lang === 'ja' && row.rangeJa ? row.rangeJa : row.range}</td>
                                    <td className="py-1.5 text-slate-700">{row.size}</td>
                                    <td className="py-1.5 pr-3 text-xs text-slate-500">{lang === 'ja' && row.commentJa ? row.commentJa : row.comment}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
            ...
            <div>
                <h4 className="text-sm font-bold text-slate-700 flex items-center gap-2 mb-1">
                    <Cable size={16} className="text-sky-500" /> {t('Central venous line', '中心静脈ライン')}
                </h4>
                <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                        <thead className="bg-slate-50 text-[10px] uppercase text-slate-500 font-bold">
                            <tr>
                                <th className="text-left py-1 pl-3">{t('Weight', '体重')}</th>
                                <th className="text-left py-1">{t('Catheter', 'カテーテル')}</th>
                                <th className="text-left py-1 pr-3">{t('Comment', 'コメント')}</th>
                            </tr>
                        </thead>
            ...
            <div className="bg-sky-50 border border-sky-200 rounded p-3 flex justify-between items-center">
                <div>
                    <div className="text-[10px] uppercase font-bold text-sky-700">{t('CVL insertion depth (height-based)', 'CVL 挿入深さ(身長ベース)')}</div>
                    <div className="text-[11px] text-sky-700 font-mono">
                        {h ? `${t('Height', '身長')} ${h} cm → ${h < 100 ? 'Height/10 − 1' : 'Height/10 − 2'}` : t('Enter patient height', '患者の身長を入力')}
                    </div>
                </div>
            ...
            <div className="bg-slate-50 border border-slate-200 rounded p-3">
                <div className="text-[10px] uppercase font-bold text-slate-500 mb-1">{t('Pearls', 'パール')}</div>
